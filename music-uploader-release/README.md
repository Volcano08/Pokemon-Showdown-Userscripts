...
This does the actual audio conversion. FFmpeg itself only distributes source code, not an official installer for any OS, which is why the easiest route differs by platform.

**Windows**

Easiest: use [winget](https://learn.microsoft.com/en-us/windows/package-manager/) (built into Windows 10/11 already):

```
winget install --id=Gyan.FFmpeg -e
```

This handles the download and PATH setup automatically. Close and reopen your terminal afterward for the PATH change to take effect.

If winget isn't available (older Windows, or it's blocked on a managed machine), do it manually instead:
...
2. Extract the zip, then move the extracted folder somewhere permanent, e.g. `C:\ffmpeg`
3. Add the `bin` folder inside it to your PATH: Start → search "environment variables" → **Edit the system environment variables** → **Environment Variables** button → under "System variables" find **Path** → **Edit** → **New** → paste the full path (e.g. `C:\ffmpeg\bin`) → OK on all the windows
4. Close and reopen your terminal

**macOS**

Easiest: use [Homebrew](https://brew.sh/):

```
brew install ffmpeg
```

If you don't have Homebrew yet, install it first with the command on brew.sh's homepage (a single line you paste into Terminal), then run the line above.
...
Open a fresh terminal/command prompt and run:

```
ffmpeg -version
```

If that prints a version number instead of "command not found," it's on your PATH and ready to go.
