...
(function () {
	...
	function getGen() {
		// Use the calc's own gen variable directly — most reliable
	...
	function loadIntoPanel(species, teamName) {
		...
		script.textContent = '(function() {' +
			...
			'function doLoad() {' +
				...
				'}' +
				// The "Custom sets only" checkbox has its own click handler
				...
				'}' +
				// Use the calc's own loadPreset which handles val + change + display
				'loadPreset("#p1", setId);' +
			'}' +
	...
	function importSet(data) {
		...
		try {
			...
			if (!alreadyExists) {
				// Fill form and save — suppress the alert("Set(s) saved.")
