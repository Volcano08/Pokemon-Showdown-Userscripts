...
(function () {
	...
	// through `win` everywhere in this file. This wasn't necessary in any
	// of the five original scripts individually - they all had @grant none,
	// which runs unsandboxed with direct page access. The moment any part
	// of this merged script needs a real @grant (GM_openInTab, for the
	// Teambuilder Mode tabs below), Violentmonkey sandboxes the whole
	// script and bare `window` stops being the page's actual window.
	// Standard web-platform globals (document, MutationObserver, Audio,
	// Node, Date) don't need this - they're equally available in the
	// sandbox. jQuery ($) is left bare throughout, matching how the
	// original Battle Update script already used it successfully under
	// this exact sandboxing.
	...
	// live from Showdown's own Options popup and persists in localStorage -
	...
	// here. (If you're looking at the source and wondering why this is
	// empty: shipping a public script with dozens of preloaded commercial
	// tracks baked in would mean distributing that audio to anyone who
	// installs it, which is a very different thing from using it yourself.)
	var MUSIC_TRACKS = [
	...
	(function () {
		...
		// __cmActiveAudio never gets cleared when a battle ends - it's the
		// same <audio> element reused across battles, and there was nothing
		// resetting it. Checking "does it exist" was really only checking
		// "has there been at least one battle so far this session", not
		// "is a battle happening right now" - so after your first battle,
		// Play/Loop would immediately switch and play audibly even from the
		// home screen. Check for a currently-open, unfinished battle room
		// instead.
		...
		var tryInit = function () {
			...
			// the page's real BattleSound object - the same reason the
			// original @grant-none version of this script used window.eval:
			// Showdown's own code needs to see the patched loadBgm on the
			// actual object it already holds a reference to. Everything
			// inside this template string executes in that real page
			// context, so bare BattleSound/window references in here are
			// correct as written and don't need win. prefixing themselves.
			win.eval(
				"const getVolume = () => {\n" +
	...
	(function () {
		...
		// the same class the Move-to-Folder script's folder picker uses - so this
		...
		var createCheatsheet = function () {
			...
			var rows = [
				['Ctrl+Alt+H', 'Home'],
				['Ctrl+Alt+B', 'Battles tab'],
				['Ctrl+Alt+T', 'Teambuilder'],
				['Ctrl+Alt+L', 'Ladder'],
				['Ctrl+Alt+F', 'Find battle'],
				['Ctrl+Alt+M', 'Music picker'],
				['Ctrl+Alt+=/-/0', 'Text size'],
				['\u2190 \u2192', 'Cycle tabs'],
				['\u2191 \u2193', 'Music volume'],
		...
		window.addEventListener('keydown', function (e) {
			...
			if (e.ctrlKey && e.altKey) {
				switch (e.key.toLowerCase()) {
					...
					case 't': e.preventDefault(); if (win.app) win.app.tryJoinRoom('teambuilder'); break;
		...
		var applyCustomAvatar = function () {
			...
			var replaceSprites = function () {
				...
				// .yours class, since both players show side by side there. Which
				// one is "mine" isn't determined by side or the flip transform
				// (confirmed inconsistent - whichever name comes first in the room
				// title sits on the left and gets flipped, regardless of who that
				// is), so it has to be matched by column position against my own
				// name in the row above. Confirmed via testing: an opponent whose
				// avatar simply hasn't resolved yet server-side shows as
				// unknownf.png regardless of this code - that's not a bug here.
	...
	// TEXT SIZE
	// ============================================================
	// Showdown has no built-in setting for this (verified against the
	// actual Options popup source - Graphics/Chat sections have nothing
	// for font size). Scales .chat-log/.battle-log/.chat/.notice
	// specifically (the actual message-text containers/classes), not the
	// whole page, so buttons/UI chrome stay their normal size. Anchored to
	// Showdown's own real base sizes (9pt for .chat/.notice, 8pt for .chat
	// small, verified in battle-log.css) rather than a bare percentage, so
	// 100% exactly matches Showdown's native look with no drift.
	(function () {
