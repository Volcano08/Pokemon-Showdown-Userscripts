...
What every feature in these two scripts actually does. For installation and configuration, see the setup guide instead.
...
- A popup about IP/proxy locks closes itself automatically (matched by its actual message text, not just its CSS class, so it can't accidentally close an unrelated popup).
...
- Your custom avatar is applied everywhere: the home screen, the top-right userbar, your name popup, your side of the battle screen, and the Best-of-3 tracker room's side-by-side player display (correctly figuring out which of the two sprites shown there is actually yours, even though the layout doesn't consistently put "you" on the same side).
...
- The all-teams view groups everything sensibly: pinned folder(s) first, then other folders alphabetically, then a "No Folder" separator, then unfoldered teams last — rather than teams appearing in whatever order Showdown happened to store them.
